...
document.addEventListener("DOMContentLoaded", function() {
    ...
    favoritesButton.addEventListener('click', (event) => {
        ...
        // Rimuove un errore precedente se presente
        const previousError = document.querySelector('.errore');
        if (previousError) {
            previousError.remove();
        }

        // Crea e mostra un nuovo messaggio di errore se non esiste già uno
        const errorElement = document.createElement('p');
        errorElement.textContent = "Per visualizzare la tua lista preferiti devi effettuare l'accesso!";
        errorElement.classList.add('errore');

        // Trova il contenitore più vicino per posizionare il messaggio di errore più logicamente
        const container = favoritesButton.closest('.favorites-box');
        if (container) {
            container.appendChild(errorElement);
        } else {
            // Se non trova un contenitore adatto, lo aggiunge dopo il bottone
            favoritesButton.parentNode.insertBefore(errorElement, favoritesButton.nextSibling);
        }
    });
});
